...
namespace FoundationDB.Samples.Tutorials
{
	...
	public class ClassScheduling : IAsyncTest
	{
		...
		{
			return this.Subspace.Pack("class", c);
		}

		protected Slice AttendsKey(string s, string c)
		{
			return this.Subspace.Pack("attends", s, c);
		}

		protected FdbKeyRange AttendsKeys(string s)
		{
			return this.Subspace.ToRange(FdbTuple.Create("attends", s));
		}
		...
		public async Task Init(FdbDatabase db, CancellationToken ct)
		{
			...
			this.Subspace = await db.Root.CreateOrOpenAsync(FdbTuple.Create("Tutorials", "ClassScheduling"), ct: ct);
		...
		public async Task IndecisiveStudent(FdbDatabase db, int id, int ops, CancellationToken ct)
		{
			...
			for (int i = 0; i < ops && !ct.IsCancellationRequested; i++)
			{
				...
				try
				{
					if (allClasses == null)
					{
						allClasses = await db.Attempt.ReadAsync((tr) => AvailableClasses(tr), ct);
					}
					...
					switch (mood)
					{
						case "add":
						{
							...
							await db.Attempt.ChangeAsync((tr) => Signup(tr, student, @class), ct);
							myClasses.Add(@class);
							break;
						}
						case "drop":
						{
							string @class = allClasses[rnd.Next(allClasses.Count)];
							await db.Attempt.ChangeAsync((tr) => Drop(tr, student, @class), ct);
						...
						case "switch":
						{
							...
							await db.Attempt.ChangeAsync((tr) => Switch(tr, student, oldClass, newClass), ct);
