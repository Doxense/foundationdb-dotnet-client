...
namespace Doxense.Unsafe.Tests //IMPORTANT: don't rename or else we loose all perf history in TeamCity!
{
	...
	public unsafe class UnsafeHelpersFacts : DoxenseTest
	{
		...
		[Test]
		public void Test_UnsafeHelpers_Fixed16_Read()
		{
			byte[] data = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF };
			fixed (byte* ptr = data)
			{
				// Little Endian
				Assert.That(UnsafeHelpers.ReadFixed16(ptr), Is.EqualTo(0x2301));
				Assert.That(UnsafeHelpers.ReadFixed16(ptr + 1), Is.EqualTo(0x4523));
				Assert.That(UnsafeHelpers.ReadFixed16(ptr + 6), Is.EqualTo(0xEFCD));
				byte* ptr2;
				ushort res;
				ptr2 = UnsafeHelpers.ReadFixed16(ptr, out res);
				Assert.That(res, Is.EqualTo(0x2301));
				Assert.That(ptr2 - ptr, Is.EqualTo(2));

				// Big Endian
				Assert.That(UnsafeHelpers.ReadFixed16BE(ptr), Is.EqualTo(0x0123));
				Assert.That(UnsafeHelpers.ReadFixed16BE(ptr + 1), Is.EqualTo(0x2345));
				Assert.That(UnsafeHelpers.ReadFixed16BE(ptr + 6), Is.EqualTo(0xCDEF));
				ptr2 = UnsafeHelpers.ReadFixed16BE(ptr, out res);
				Assert.That(res, Is.EqualTo(0x0123));
				Assert.That(ptr2 - ptr, Is.EqualTo(2));
			}
		}

		[Test]
		public void Test_UnsafeHelpers_Fixed16_Write()
		{
			const int CAPA = 8;
			...
			fixed (byte* ptr = data)
			{
				...
				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16Unsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16Unsafe(ptr, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("23 01 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteZeroFixed16(ptr + 1);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 1 + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16(ptr, ptr + CAPA, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("23 01 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16Unsafe(ptr + 2, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("58 58 23 01 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2 + 2)));

				// Big Endian

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16BEUnsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16BEUnsafe(ptr, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("01 23 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16BE(ptr, ptr + CAPA, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("01 23 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16BEUnsafe(ptr + 1, 0);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 1 + 2)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed16BEUnsafe(ptr + 2, 0x0123);
				Assert.That(ToHex(data), Is.EqualTo("58 58 01 23 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 2 + 2)));
		...
		[Test]
		public void Test_UnsafeHelpers_Fixed32_Read()
		{
			byte[] data = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF };
			fixed (byte* ptr = data)
			{
				// Little Endian
				Assert.That(UnsafeHelpers.ReadFixed32(ptr), Is.EqualTo(0x67452301));
				Assert.That(UnsafeHelpers.ReadFixed32(ptr + 1), Is.EqualTo(0x89674523));
				Assert.That(UnsafeHelpers.ReadFixed32(ptr + 4), Is.EqualTo(0xEFCDAB89));

				byte* ptr2;
				uint res;
				ptr2 = UnsafeHelpers.ReadFixed32(ptr, out res);
				Assert.That(res, Is.EqualTo(0x67452301));
				Assert.That(ptr2 - ptr, Is.EqualTo(4));

				// Big Endian
				Assert.That(UnsafeHelpers.ReadFixed32BE(ptr), Is.EqualTo(0x01234567));
				Assert.That(UnsafeHelpers.ReadFixed32BE(ptr + 1), Is.EqualTo(0x23456789));
				Assert.That(UnsafeHelpers.ReadFixed32BE(ptr + 4), Is.EqualTo(0x89ABCDEF));
				ptr2 = UnsafeHelpers.ReadFixed32BE(ptr, out res);
				Assert.That(res, Is.EqualTo(0x01234567));
				Assert.That(ptr2 - ptr, Is.EqualTo(4));
			}
		}

		[Test]
		public void Test_UnsafeHelpers_Fixed32_Write()
		{
			const int CAPA = 8;
			byte[] data = new byte[CAPA];
			fixed (byte* ptr = data)
			{
				...
				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32Unsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 00 00 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long)(ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32Unsafe(ptr, 0x01234567);
				Assert.That(ToHex(data), Is.EqualTo("67 45 23 01 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteZeroFixed32(ptr + 1);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 00 00 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 1 + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32(ptr, ptr + CAPA, 0x01234567);
				Assert.That(ToHex(data), Is.EqualTo("67 45 23 01 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32Unsafe(ptr + 2, 0x01234567);
				Assert.That(ToHex(data), Is.EqualTo("58 58 67 45 23 01 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 2 + 4)));

				// Big Endian

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32BEUnsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 00 00 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32BEUnsafe(ptr, 0x01234567);
				Assert.That(ToHex(data), Is.EqualTo("01 23 45 67 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32BE(ptr, ptr + CAPA, 0x01234567);
				Assert.That(ToHex(data), Is.EqualTo("01 23 45 67 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32BEUnsafe(ptr + 1, 0);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 00 00 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 1 + 4)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed32BEUnsafe(ptr + 2, 0x01234567);
		...
		[Test]
		public void Test_UnsafeHelpers_Fixed64_Read()
		{
			byte[] data = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF };
			fixed (byte* ptr = data)
			{
				// Little Endian
				Assert.That(UnsafeHelpers.ReadFixed64(ptr), Is.EqualTo(0xEFCDAB8967452301));
				byte* ptr2;
				ulong res;
				ptr2 = UnsafeHelpers.ReadFixed64(ptr, out res);
				Assert.That(res, Is.EqualTo(0xEFCDAB8967452301));
				Assert.That(ptr2 - ptr, Is.EqualTo(8));

				// Big Endian
				Assert.That(UnsafeHelpers.ReadFixed64BE(ptr), Is.EqualTo(0x0123456789ABCDEF));
				ptr2 = UnsafeHelpers.ReadFixed64BE(ptr, out res);
				Assert.That(res, Is.EqualTo(0x0123456789ABCDEF));
				Assert.That(ptr2 - ptr, Is.EqualTo(8));
			}
		}

		[Test]
		public void Test_UnsafeHelpers_Fixed64_Write()
		{
			const int CAPA = 16;
			byte[] data = new byte[CAPA];
			fixed (byte* ptr = data)
			{
				...
				// Little Endian
				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64Unsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 00 00 00 00 00 00 58 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64Unsafe(ptr, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("EF CD AB 89 67 45 23 01 58 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64(ptr, ptr + CAPA, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("EF CD AB 89 67 45 23 01 58 58 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteZeroFixed64(ptr + 1);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 00 00 00 00 00 00 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 1 + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64Unsafe(ptr + 2, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("58 58 EF CD AB 89 67 45 23 01 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 2 + 8)));

				// Big Endian

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64BEUnsafe(ptr, 0);
				Assert.That(ToHex(data), Is.EqualTo("00 00 00 00 00 00 00 00 58 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64BEUnsafe(ptr, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("01 23 45 67 89 AB CD EF 58 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64BE(ptr, ptr + CAPA, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("01 23 45 67 89 AB CD EF 58 58 58 58 58 58 58 58"));
				Assert.That((long) ptr2, Is.EqualTo((long) (ptr + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64BEUnsafe(ptr + 1, 0);
				Assert.That(ToHex(data), Is.EqualTo("58 00 00 00 00 00 00 00 00 58 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 1 + 8)));

				Wipe(data); ptr2 = UnsafeHelpers.WriteFixed64BEUnsafe(ptr + 2, 0x0123456789ABCDEF);
				Assert.That(ToHex(data), Is.EqualTo("58 58 01 23 45 67 89 AB CD EF 58 58 58 58 58 58"));
				Assert.That((long)ptr2, Is.EqualTo((long)(ptr + 2 + 8)));
